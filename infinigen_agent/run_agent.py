...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="随机种子（仅在使用 --auto-generate 时有效，默认None表示随机生成）"
    )
    parser.add_argument(
        "--infinigen-root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="静态资产根目录路径（默认: infinigen/assets/static_assets/source）"
    )
    parser.add_argument(
        "--procedural",
        action="store_true",
        default=True,
        help="对缺失的家具使用程序化生成（默认启用）"
    )
    parser.add_argument(
        "--no-procedural",
        action="store_false",
        dest="procedural",
        help="禁用程序化生成，只在场景中查找已有对象"
    )
    ...
    if args.auto_generate:
        ...
        try:
            ...
            # 如果seed为None，随机生成
            import random
            seed = args.seed if args.seed is not None else random.randint(1, 1e9)
            if args.seed is None:
                print(f"ℹ 未指定seed，使用随机seed: {seed}")
    ...
    try:
        ...
        if args.render_image or args.render_video:
            ...
            results = agent.process_request_with_render(
                ...
                video_fps=args.video_fps,
                resolution=resolution,
                import_missing_assets=args.import_assets,
                static_assets_root=args.static_assets_root,
                use_procedural_generation=args.procedural
            )
            print(f"\n✓ 成功！")
