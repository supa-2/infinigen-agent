...
def apply_scene_seed(seed, task=None):
    ...
    # numpy.random.seed requires seed to be between 0 and 2**32 - 1
    # If the parsed seed exceeds this range, use modulo to wrap it
    MAX_NUMPY_SEED = 2**32 - 1
    if scene_seed > MAX_NUMPY_SEED:
        original_seed = scene_seed
        scene_seed = scene_seed % (MAX_NUMPY_SEED + 1)
        logger.warning(
            f"Seed {original_seed} exceeds numpy's maximum ({MAX_NUMPY_SEED}), "
            f"wrapping to {scene_seed}"
        )
