...
def reorganize_old_framesfolder(frames_old):
    ...
    for img_path in frames_old.iterdir():
        ...
        try:
            ...
            # 如果解析失败（例如文件名包含特殊字符如####），跳过该文件
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"无法解析文件名 {img_path.name}，跳过重组: {e}")
            continue

    if frames_dest != frames_old:
        # 检查目录是否为空，如果不为空则使用 rmtree 删除
        try:
            remaining_files = list(frames_old.iterdir())
            if remaining_files:
                logger.warning(f"目录 {frames_old} 中仍有 {len(remaining_files)} 个文件/目录未移动，使用 rmtree 删除")
                shutil.rmtree(frames_old)
            else:
                frames_old.rmdir()
        except OSError as e:
            # 如果 rmdir 失败，尝试使用 rmtree
            logger.warning(f"无法删除目录 {frames_old}，尝试使用 rmtree: {e}")
            try:
                shutil.rmtree(frames_old)
            except Exception as e2:
                logger.error(f"无法删除目录 {frames_old}: {e2}")
